...
export default class XRTRig {
  ...
  init(camera_el_) {
    ...
    this.el_.setObject3D('mesh', new THREE.Mesh(new THREE.ConeBufferGeometry(0.5, -0.5, 9, 1, true),
      new THREE.MeshBasicMaterial({
        color: 'skyblue', opacity: 0, side: THREE.FrontSide,
        transparent: true
      })));
  ...
  pressed_() {
    ...
    for (let info of this.info_els_) { info.setAttribute("animation", INFO_ANIMATION_FADEIN); }
  ...
  released_() {
    ...
    for (let info of this.info_els_) { info.setAttribute("animation", INFO_ANIMATION_FADEOUT); }
