...
import { AframeGlyphRenderer } from './AframeGlyphRenderer.mjs'; // WebGL shader and color
import { LinkRenderLayer } from './renderLayer/LinkRenderLayer.mjs'; // likifier? URL and underline
import { CursorRenderLayer } from './renderLayer/CursorRenderLayer.mjs'; // Cursor properties looking / animation / rendercallback
import { acquireCharAtlas } from './atlas/CharAtlasCache.mjs'; // texture atlas (has webglcharatlas)
import { WebglCharAtlas } from './atlas/WebglCharAtlas.mjs'; // Browser canvas / WebGL texture / fontstyle etc.
import { RectangleRenderer } from './RectangleRenderer.mjs'; // WebGL shader and color
import { IWebGL2RenderingContext } from './Types.mjs'; // ts typing? WebGL accessor
import { RenderModel, COMBINED_CHAR_BIT_MASK, RENDER_MODEL_BG_OFFSET, RENDER_MODEL_FG_OFFSET, RENDER_MODEL_INDICIES_PER_CELL } from './RenderModel.mjs'; // selection and FG/BG something
import { Disposable } from 'common/Lifecycle.mjs'; // register / unregister and dispose array
import { NULL_CELL_CODE } from 'common/buffer/Constants.mjs'; // common constants for font highlight and coloring
import { Terminal, IEvent } from 'xterm'; // base xterm (no webgl) but not confirmed yet XXX
import { IRenderLayer } from './renderLayer/Types.mjs'; // definition of xterm, color and renderdimensions(font size) (no WebGL)
import { IRenderDimensions, IRenderer, IRequestRedrawEvent } from 'browser/renderer/Types.mjs'; // definition of xterm, browser event and redraw count(start/end)
import { ITerminal, IColorSet } from 'browser/Types.mjs'; // facade class (HTML, browser, buffer, camena, option, link, event) and color
import { EventEmitter } from 'common/EventEmitter.mjs'; // event queue
import { CellData } from 'common/buffer/CellData.mjs'; // terminal cell
